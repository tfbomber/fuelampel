...
export function computeDecision(
  ...
  if (levelPercent <= 15) {
    return {
      ...
      reason: `🔴 Tank critically low (≤15%). Refuel now!`,
      readiness: 'Action',
      zone: 'Critical',
      confidenceLevel,
    };
  ...
  if (levelPercent < 30) {
    if (isGoodWindow || isGoodDeal) {
      return {
        ...
        station,
        saving_estimate: savingVsMedian,
        reason: isGoodDeal ? `🟢 Great price today! Fill up on your way.` : `🟡 Evening price window active — good time to top up.`,
        readiness: 'Action',
        zone: 'Low',
        confidenceLevel,
      };
      ...
      return {
        ...
        station,
        saving_estimate: savingVsMedian,
        reason: `🟡 Tank is low — price is average. Keep an eye on it.`,
        readiness: 'Monitor',
        zone: 'Low',
        confidenceLevel,
      };
    ...
    return {
      ...
      station,
      saving_estimate: savingVsMedian,
      reason: `🟢 Great price today! ${(GOOD_DEAL_PCT_THRESHOLD * 100).toFixed(0)}% cheaper than the regional median.`,
      readiness: 'Monitor',
      zone: 'Planning',
      confidenceLevel,
    };
    ...
    return {
      ...
      station,
      saving_estimate: savingVsMedian,
      reason: `Tank is healthy and no standout price advantage found.`,
      readiness: 'NotNeeded',
      zone,
      confidenceLevel,
    };
